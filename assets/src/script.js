...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (customerPanelBtn) {
        customerPanelBtn.addEventListener('click', async function(e) {
            ...
            // Navigate to customer panel page
            window.location.href = 'assets/src/customer-panel.html';
        });
    }

    function showDiscordRedirectModal(discordInvite) {
        // Create Discord redirect modal
        ...
        modalContent.innerHTML = `
            <div style="font-size: 3rem; margin-bottom: 1rem; color: #5865F2;">
            ...
            <p style="color: var(--text-secondary); margin-bottom: 2rem; line-height: 1.6;">
                For account management, license support, downloads, and community assistance, 
                please join our Discord server and contact our support team!
            </p>
            <div style="display: flex; gap: 1rem; justify-content: center;">
                ...
                <button id="close-discord-modal" style="
        ...
        // Event listeners
        const joinDiscordBtn = modal.querySelector('#join-discord');
        const closeBtn = modal.querySelector('#close-discord-modal');
...
class PackageManager {
        ...
        const pricingHTML = Object.keys(pkg.pricing).map(priceKey => {
            ...
            return `
                <div class="pricing-option${popularClass}" data-package="${packageKey}" data-duration="${priceKey}">
                    ...
                    <button class="buy-btn" onclick="packageManager.addToCart('${packageKey}', '${priceKey}')">
                        Add to Cart
                    </button>
                </div>
            `;
    ...
    addToCart(packageKey, durationKey) {
        try {
            const pkg = this.packagesData.packages[packageKey];
            const pricing = pkg.pricing[durationKey];
            
            if (!pkg || !pricing) {
                console.error('Package or pricing not found:', packageKey, durationKey);
                return;
            }

            // Create cart item
            const cartItem = {
                id: `${packageKey}-${durationKey}`,
                name: `${pkg.name} - ${pricing.duration}`,
                price: pricing.price,
    ...
    showAddToCartFeedback() {
        ...
        feedback.className = 'cart-success-feedback';
        feedback.innerHTML = `
    ...
    async handlePurchase(packageKey, durationKey) {
        // Legacy method - redirect to addToCart
        this.addToCart(packageKey, durationKey);
    }
